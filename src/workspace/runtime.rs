...
impl RuntimeHolder {
    pub fn new(runtime_dir : &RuntimeDir, rootfs_config: Option<&RootfsConfig>) -> io::Result<RuntimeHolder> {
        ...
        if let Some(config) = rootfs_config {
            with_rootfs = true;
            work_dir = Some(runtime_dir.parent()
                .expect("runtime dir should not be /")
                .join("work"));
            upper_dir = Some(runtime_dir.parent()
                .expect("runtime dir should not be /")
                .join("upper"));

            fs::create_dir(&work_dir.as_ref().unwrap())?;
            fs::create_dir(&upper_dir.as_ref().unwrap())?;
            ...
            nix::mount::mount(
                ...
                |_| io::Error::new(io::ErrorKind::Other, "mount runtime overlay failed!")
            )?;
        }
    ...
    fn drop(&mut self) {
        ...
        if self.with_rootfs {
            if let Err(e) = nix::mount::umount(&self.runtime_dir) {
                log::debug!("Error when umount runntime_dir {}, err: {}", self.runtime_dir.display(), e);
            }
            ...
            if let Err(e) = fs::remove_dir_all(work_dir) {
                log::debug!("Error when remove work dir {}, err: {}", work_dir.display(), e);
            }
            if let Err(e) = fs::remove_dir_all(upper_dir) {
                log::debug!("Error when remove upper dir {}, err: {}", upper_dir.display(), e);
            }
        }

        if let Err(e) = fs::remove_dir_all(&self.runtime_dir) {
            log::debug!("Error when remove runtime dir {}, err: {}", self.runtime_dir.display(), e);
        }
    }
}
